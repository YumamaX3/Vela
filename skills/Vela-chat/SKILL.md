---
name: vela-chat
description: Chat / code generation via Vela using OpenAI /v1/chat/completions or Anthropic /v1/messages format with streaming + auto-fallback combos. Use when the user wants to ask an LLM, generate code, summarize text, or run prompts through Vela.
---
...
- `POST $VELA_URL/v1/chat/completions` — OpenAI format
- `POST $VELA_URL/v1/messages` — Anthropic format

## Discover

```bash
curl $VELA_URL/v1/models | jq '.data[].id'
# Per-model metadata (contextWindow, params)
curl "$VELA_URL/v1/models/info?id=openai/gpt-4o"
```
...
```bash
curl -X POST $VELA_URL/v1/chat/completions \
...
const client = new OpenAI({ baseURL: `${process.env.VELA_URL}/v1`, apiKey: process.env.VELA_KEY });
...
```bash
curl -X POST $VELA_URL/v1/messages \
  -H "Authorization: Bearer $VELA_KEY" \
