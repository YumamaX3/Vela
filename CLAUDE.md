...
├── package.json              # v0.9.93 — bump with every release
...
├── src/
│   ├── app/                  # Next App Router — 30 dashboard pages (35 page.js total) + 198 API routes
...
├── tests/                    # 393 test files — unit 355 · translator 27 · contract 10 · auth 1 (+4 .snap)
├── scripts/                  # changelog sync, standalone assets, i18n seed, icon subset (gitignored except 2)
...
|-|-|-|
| **Dashboard** | `src/app/` | The UI — 30 dashboard pages (35 `page.js` total), 198 API routes |
...
- **`registry/`** — one file per provider (**166 files on disk**, 149 imported by the generated index → 111 dialable chat transports). `registry/index.js` is **auto-generated** — never hand-edit.
...
- **Runner**: vitest, `tests/` root. **393 test files** (unit 355 · translator 27 · contract 10 · auth 1) plus **4 `.snap` fixtures**; vitest's `include` is `**/*.test.{js,jsx}`, so it collects **392** — the one stray, `unit/custom-server-h2c.test.cjs`, runs under `node:test`, not vitest. Canonical invocation: `npx vitest run -c tests/vitest.config.js` — there is no root config and no `test` script, and without `-c` the `@/` alias dies.
...
| **v0.9.90** | **The Mended Word** 🐛 | v0.9.89 named this exact class - a name declared and never resolvable - and closed three of its four instances; the fourth it closed **partly**. `ScopePill` took the new prop in its signature while its body still read `key.allowedModels`, so the card lens and the detail drawer died on first render: `Runtime ReferenceError: key is not defined` at `KeyBits.js:61`. **Why no instrument saw it**: a bare identifier is a parse *success* and a runtime *failure*, so the bundler had nothing to refuse - and the suite passed because **no test had ever rendered these components at all**. The room was covered by nothing. Mended the last old word, then left the instrument that keeps it: `tests/unit/key-pills-render.test.jsx` (new) renders the fleet's smallest marks in happy-dom exactly as the room does - `3 models` against `1 model`, the scope pill's tooltip spelling the model ids, `All models` for an honest absent scope, `60 RPM` / `100K tok` / `$5` for the ceilings set, nothing at all when none are. **Mutation-proven**: restoring the old name reddens the four `ScopePill` cases with the identical `ReferenceError` while the two `LimitPills` cases stay green; the mend returns **6/6**. Found by walking the real room in a real browser - the second defect in two tides that a green build and a green suite both tolerated. **Proof**: pill suite 6 cases green, red-then-green under mutation; `npm run build` green at v0.9.90 · the hull landed whole — `ghcr.io/yumamax3/vela:0.9.90` **HTTP 200** as a real OCI index (`linux/amd64` + `linux/arm64`, **no arm64 stall this tide**) in run `35801700282`, success in ~21m48s, `:latest` carried with it · Release published, `draft=false`. |
| **v0.9.89** | **The Unspoken Names** 🐛 | Three times in one tide, one class of wound: a name declared and never resolvable. v0.9.88 shipped a green build and a green suite - and a key room that answered **500 on every door** - because a named import whose target module does not export it is **tolerated by webpack and refused by Turbopack**, so only a real browser walking a real server saw the harbor dark. Four seams mended: `endpoint/lib/keyFormat.js`'s re-export block carried `limitsFromRecord` and not `limitsToRecord`, though `keyLimits.js:54` exports both as a pair (four files imported the missing name - one module graph, five apparent 500s: the room *and* `/api/keys/{stats,export,bulk,import}`); `ScopePill`/`LimitPills` each took a prop named literally `key`, which React **reserves and strips** before the component is called, so the prop arrived undefined and the card lens died (renamed `k` across two signatures and four call sites - the room's own convention - and that same reservation had been painting `Encountered two children with the same key, [object Object]` twice per render for a whole release); `KeyRail.js:34` read `keys` off `deck`, which returns fifty-two names and not that one (it lives in `c`, exactly as `KeyFleetPulse` already read it); and `api/providers/[id]/models/route.js:15` imported `resolveClineModels`, a name appearing **nowhere else** in `src/` or `open-sse/`. v0.9.88's recorded debt paid in the same current: `add_link` joined the inventory (240 → 241) and the GSUB-pruned subset was **re-minted** - `vela-icons.05aae2a6573832c1.woff2` (184,604 bytes), manifest rewritten, `globals.css`'s `@font-face` pointed at the new identity. **Proof**: live browser walk - rail (`All keys 1 · Uncategorized 1`), card pills (`Active · All models · stored here`) and the drawer all render with **0 page errors, 0 reserved-prop warnings**; `/api/keys/{stats,export,usage}` **200** and `{bulk,import}` **405**; `icon-subset` green (2/2) where it was red at HEAD; `npm run build` green at v0.9.89. |
