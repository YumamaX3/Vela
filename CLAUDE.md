...
├── tests/                    # 355 .test.js + 9 .test.jsx — unit 317 · translator 27 (19 + real/ 8) · contract 10 · auth 1
...
- **Runner**: vitest, `tests/` root. **356 `.test.js` + 9 `.test.jsx`** (unit 318 · translator 27 (19 + `real/` 8) · contract 10 · auth 1). Canonical invocation: `npx vitest run -c tests/vitest.config.js` — there is no root config and no `test` script, and without `-c` the `@/` alias dies.
