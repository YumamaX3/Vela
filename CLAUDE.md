...
- **Runner**: vitest, `tests/` root. **368 `.test.js` + 9 `.test.jsx`** (unit 330 · translator 27 (19 + `real/` 8) · contract 10 · auth 1). Canonical invocation: `npx vitest run -c tests/vitest.config.js` — there is no root config and no `test` script, and without `-c` the `@/` alias dies.
