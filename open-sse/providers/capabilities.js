...
export const MODEL_CAPABILITIES = {
  ...
  // Claude Fable 5.1 — permanently adaptive (cannot disable thinking), 1M context (W2, v0.9.48)
  ...
  // GLM 5.3 Flash: natively multimodal — video + pdf inputs, 1M context
  // (ported from upstream 9router 9c650e1d — W3, v0.9.49)
  ...
  // DeepSeek's first V4 model with image input; text limits match V4-Flash (W3, v0.9.49)
  ...
  // Nex N2.5 Pro — vision per the Jerouter V2 live catalog (2026-09-10); the id
  // matches no glob pattern, so without this exact pin it resolves text-only.
  // N2.5 Mini stays text per the same catalog.
  "nex-n2.5-pro":      { vision: true },
  ...
  // NOTE: "union-alpha" is NOT here on purpose. Upstream declares it in this
  // global table (their line 145), but upstream has no Jerouter: a global row
  // reaches every provider serving that id, and Vela's second lane resells it
  // text-only. Measured: the global row turned Jerouter's union-alpha vision and
  // failed jerouter-catalog.test.js:49. It lives in the "opencode" block below.
};
...
export const PROVIDER_CAPABILITIES = {
  ...
  // CodeBuddy intl — same gateway catalog as CN. The openai-style
  // reasoning_effort format matters here: the generic *deepseek-v4* pattern
  // would otherwise pick the vendor-native "deepseek" thinking shape, which
  // the CodeBuddy gateway does not accept.
  "codebuddy-intl": {
  ...
  // Measured windows and thinkingFormat (upstream v0.5.81) ride the entries
  // themselves; the free-suffix pattern below covers ids no table enumerates.
  "opencode": {
    "muse-spark-1.2":                  { vision: true, reasoning: true, tools: true, thinkingFormat: "openai", contextWindow: 1048576, maxOutput: 131072 },
    "muse-spark-1.3":                  { vision: true, reasoning: true, tools: true, thinkingFormat: "openai", contextWindow: 1048576, maxOutput: 131072 },
    "muse-spark-1.2-contributor-free": { vision: true, reasoning: true, tools: true, thinkingFormat: "openai", contextWindow: 1048576, maxOutput: 131072 },
    "muse-spark-1.3-contributor-free": { vision: true, reasoning: true, tools: true, thinkingFormat: "openai", contextWindow: 1048576, maxOutput: 131072 },
    // OpenCode Free Union Alpha — multimodal (text+vision), 262K context, 131K
    // max output (upstream v0.5.81). Scoped here for the same reason as its
    // muse-spark siblings: Jerouter resells the SAME id as text-only (its own
    // annotation, pinned by jerouter-catalog.test.js), so this truth must not
    // ride a global row. Rides the Zen lane's OpenAI + Claude transports alike.
    "union-alpha":                     { vision: true, contextWindow: 262144, maxOutput: 131072 },
  },
  "opencode-go": {
    "muse-spark-1.2-contributor":      { vision: true, reasoning: true, tools: true, thinkingFormat: "openai", contextWindow: 1048576, maxOutput: 131072 },
    "muse-spark-1.3-contributor":      { vision: true, reasoning: true, tools: true, thinkingFormat: "openai", contextWindow: 1048576, maxOutput: 131072 },
  },
...
export const PATTERN_CAPABILITIES = [
  ...
  // Grok 4.6: 500k context, no text output limit (docs.x.ai/developers/grok-4-6) (W3, v0.9.49)
  ...
  // ── OpenCode Free Muse Spark — the free lane's future ids ────────────────
  // Scoped to the "-contributor-free" suffix on purpose: Jerouter resells the
  // bare "*muse-spark*<contributor*" ids as text-only, pinned by
  // jerouter-catalog.test.js, so the pattern must not reach past the Zen lane.
  { pattern: "*muse-spark*contributor-free*", caps: { vision: true, reasoning: true, thinkingFormat: "openai", contextWindow: 1048576, maxOutput: 131072 } },
...
export function getCapabilitiesForModel(provider, model) {
  ...
  // 4. Floor — still refined: the synced catalog knows models no table enumerates.
