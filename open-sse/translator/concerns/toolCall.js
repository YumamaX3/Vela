...
// `bash`, `web_search_20250305`) are passed through untouched (ported from upstream
// 9router — W2, v0.9.48).
//
...
// Only gateways that declare the `requireClaudeToolType` quirk (MiniMax) reject
// typeless tools. Applying the default globally breaks Claude-format endpoints
// that only accept the legacy typeless tool shape — DeepSeek's Anthropic-
// compatible endpoint answers HTTP 400 "unknown variant `custom`" and every
// Claude Code request routed there fails (upstream 9router #3905; the W2
// port shipped this default unscoped — ADR-004 wound-1, fixed by this rebase).
