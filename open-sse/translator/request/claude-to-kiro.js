 ...
 *
 * It also handles the Vela-synthetic `-agentic` / `-thinking` suffixes and
...
export function claudeToKiroRequest(model, body, stream, credentials) {
  ...
  // contentPrefix in the first user turn). Carried non-enumerable (as Vela's
  // _kiroUpstreamModel tag just below) so result.systemPrompt stays readable
  // for the replay key while JSON.stringify/structuredClone omit it from the
  // wire. (upstream 1892ed77 destination fix, Vela's carrier — ADR-004 M2)
