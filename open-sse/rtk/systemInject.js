// Shared system-prompt injector: appends (or prepends, per Vela's v0.9.19
// position param) an instruction into the system message of the final request
// body, dispatching by format so it works for translated and native-passthrough
// flows. Used by caveman.js, ponytail.js, and user-defined prompt injectors.
...
export function injectSystemPrompt(body, format, prompt, position = "append") {
  try {
    ...
    if (typeof body.instructions === "string") {
      injectInstructionsSystem(body, prompt, position);
      return;
    }
    if (Array.isArray(body.messages)) {
      injectChatSystem(body, prompt, position);
      return;
    }
    if (Array.isArray(body.input)) {
      // Responses input[]: empty array already normalized elsewhere; string stays untouched here
      injectResponsesInputSystem(body, prompt, position);
      return;
    }
...
function mergeString(curr, prompt, position) {
  if (!curr) return prompt;
  if (hasPrompt(curr, prompt)) return curr;
  return position === "prepend" ? `${prompt}${SEP}${curr}` : `${curr}${SEP}${prompt}`;
}

// ---- OpenAI instructions string ----
function injectInstructionsSystem(body, prompt, position) {
  try {
    const curr = body.instructions;
    if (typeof curr !== "string") return;
    if (hasPrompt(curr, prompt)) return;
    const next = mergeString(curr, prompt, position);
    try { body.instructions = next; } catch (_) { /* frozen/proxy fail-open */ }
  } catch (_) {}
}

// ---- Chat messages[] ----
function injectChatSystem(body, prompt, position) {
  try {
    ...
    if (idx >= 0) {
      appendToChatMessage(arr[idx], prompt, position);
    } else {
...
function appendToChatMessage(msg, prompt, position) {
  try {
    if (!msg || typeof msg !== "object") return;
    const c = msg.content;
    if (typeof c === "string") {
      const next = mergeString(c, prompt, position);
      if (next === c) return;
...
// ---- Responses input[] ----
function injectResponsesInputSystem(body, prompt, position) {
  try {
    ...
    if (idx >= 0) {
      appendToResponsesMessage(arr[idx], prompt, position);
    } else {
...
function appendToResponsesMessage(msg, prompt, position) {
  try {
    if (!msg || typeof msg !== "object") return;
    const c = msg.content;
    if (typeof c === "string") {
    ...
    if (Array.isArray(c)) {
      ...
      const block = { type: RESPONSES_ITEM.INPUT_TEXT, text: prompt };
      try { if (position === "prepend") c.unshift(block); else c.push(block); } catch (_) {}
      return;
    }
...
// ---- Claude ----
function injectClaudeSystem(body, prompt, position) {
  try {
    const sys = body.system;
    if (typeof sys === "string") {
      if (hasPrompt(sys, prompt)) return;
      const next = mergeString(sys, prompt, position);
    ...
    if (Array.isArray(sys)) {
      ...
      try {
        if (position === "prepend") sys.unshift(block);
        else {
          let lastCacheIdx = -1;
          try {
...
// ---- Gemini ----
function injectGeminiSystem(body, prompt, position) {
  try {
    let target = body;
    try {
    ...
    if (sys && Array.isArray(sys.parts)) {
      ...
      try { if (position === "prepend") sys.parts.unshift(part); else sys.parts.push(part); } catch (_) {}
      return;
    }
