...
export function hasUnicodePropertyEscape(pattern) {
  // Fast path (ADR-004 upgrade-as-you-fold, perf law): a property escape
  // literally begins with a backslash, so a value carrying none can never
  // match — skip the regex entirely. Behavior-identical to the test below;
  // it bounds the per-pattern cost of clean schemas to a single scan.
  if (typeof pattern !== "string" || !pattern.includes("\\")) return false;
  return UNICODE_PROPERTY_ESCAPE.test(pattern);
}
