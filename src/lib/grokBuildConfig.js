...
const previousDefaultRegExp = /^# Vela-prev-default = "([^"]*)"[ \t]*\r?\n?/m;
const previousSubagentRegExp = (type) =>
  new RegExp(
    `^# Vela-prev-subagent-${escapeRegExp(type)} = "([^"]*)"[ \\t]*\\r?\\n?`,
    "m",
  );
...
function buildModelSection({ slot, model, baseUrl, apiKey, contextWindow, name }) {
  const lines = [
    ...
    `description = ${tomlString("Routed via Vela gateway")}`,
...
function rememberPreviousDefault(toml) {
  ...
  return insertMarker(toml, `# Vela-prev-default = ${tomlString(current)}\n`);
}
...
function rememberPreviousSubagent(toml, type) {
  ...
  return insertMarker(
    toml,
    `# Vela-prev-subagent-${type} = ${tomlString(previous)}\n`,
  );
}
