// Storage Covenant Wave A8 — the mysql twin of sqlite/pricingRepo.js.
// The three-stratum merge (static _canonical → synced → user-sovereign) and
// the Pricing Covenant (C4: the 'pricing_sync' scope is never mixed with the
// user's 'pricing' scope) carry over byte-for-byte; only the kv plumbing
// changes harbors (makeKvMysql + backticked `key` + VALUES(col) upsert).
...
const pricingKv = makeKvMysql("pricing");
const syncKv = makeKvMysql("pricing_sync"); // Pricing Covenant — never mixed with user overrides
...
export async function updatePricing(pricingData) {
  ...
  await db.transaction(async (tx) => {
    for (const [provider, models] of Object.entries(pricingData)) {
      ...
      await tx.run(
        `INSERT INTO kv(scope, \`key\`, value) VALUES('pricing', ?, ?) ON DUPLICATE KEY UPDATE value = VALUES(value)`,
