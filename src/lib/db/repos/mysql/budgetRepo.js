// Usage Observatory W3-A — quotaRepo, the mysql twin harbor.
// Same contract as sqlite/budgetRepo.js; only the kv plumbing changes harbor
// (makeKvMysql + backticked `key`), byte-for-byte otherwise. Budget
// definitions ride the kv store (scope "budgets") — the sealed plan reserves
// migration 009 for W4 saved views, so W3's budget engine persists its
// definitions as CONFIG, not a new table.
...
const SCOPE = "budgets";
const budgetsKv = makeKvMysql(SCOPE);

// Hot-path TTL: keyGate reads the budget list on every /v1 authorization
// (W3-B). A 5s cache matches the spend stage's BUDGET_CACHE_TTL_MS so the
// gate stays fast under load.
