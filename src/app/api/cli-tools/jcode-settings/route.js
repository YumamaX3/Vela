...
const getProviderEnvPath = () => {
  ...
  return path.join(configDir, "jcode", "provider-Vela.env");
};
...
const hasVelaConfig = (config) => {
  ...
  if (providers["vela"]) return true;

  for (const [name, provider] of Object.entries(providers)) {
    if (provider.base_url && provider.base_url.includes("localhost:32060")) {
      return true;
    }
  }
...
export async function GET() {
  ...
  const hasVela = hasVelaConfig(config);
...
export async function POST(request) {
  try {
    ...
    env.JCODE_VELA_API_KEY = apiKey;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "jcode configured successfully. Use: jcode --provider-profile Vela",
      configPath: getConfigPath(),
    });
  } catch (error) {
...
export async function DELETE() {
  try {
    ...
    delete config.providers["vela"];

    await writeConfig(config);

    const env = await readProviderEnv();
    delete env.JCODE_VELA_API_KEY;
    await writeProviderEnv(env);

    return NextResponse.json({
      success: true,
      message: "Vela configuration removed from jcode",
    });
  } catch (error) {
