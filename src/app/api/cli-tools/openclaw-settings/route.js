...
// Check if settings has Vela config
const hasVelaConfig = (settings) => {
  if (!settings || !settings.models || !settings.models.providers) return false;
  return !!settings.models.providers["vela"];
};

// Read per-agent models.json and return current model id (without "Vela/" prefix)
const readAgentModel = async (agentDir) => {
  try {
    const modelsPath = path.join(agentDir, "models.json");
    const content = await fs.readFile(modelsPath, "utf-8");
    const data = JSON.parse(content);
    const models = data?.providers?.["vela"]?.models;
...
// POST - Update Vela settings (merge with existing settings)
export async function POST(request) {
  try {
    ...
    const fullModelId = `Vela/${model}`;

    // Remove all old Vela/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("Vela/"))
    ...
    // Add fresh Vela models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`Vela/${m}`] = {};
    });

    // Remove old Vela model from each agent in agents.list. The
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        if (resolveAgentModel(agent.model).startsWith("Vela/")) {
    ...
    // Update models.providers.Vela with all models
    settings.models.providers["vela"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent) => {
        ...
        if (agentModel) return { ...agent, model: `Vela/${agentModel}` };
        return agent;
      });
...
// DELETE - Remove Vela settings only (keep other settings)
export async function DELETE() {
  try {
    ...
    // Remove Vela from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["vela"];
    ...
    // Remove Vela models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("Vela/"));
    ...
    // Reset agents.defaults.model.primary if it uses Vela
    if (settings.agents?.defaults?.model?.primary?.startsWith("Vela/")) {
    ...
    return NextResponse.json({
      success: true,
      message: "Vela settings removed successfully",
    });
  } catch (error) {
