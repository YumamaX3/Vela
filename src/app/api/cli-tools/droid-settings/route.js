...
// Check if settings has Vela customModels
const hasVelaConfig = (settings) => {
  if (!settings || !settings.customModels) return false;
  return settings.customModels.some(m => m.id?.startsWith("custom:Vela"));
};
...
// POST - Update Vela customModels (merge with existing settings)
...
export async function POST(request) {
  try {
    ...
    // Remove all existing Vela configs
    settings.customModels = settings.customModels.filter(m => !m.id?.startsWith("custom:Vela"));
...
// DELETE - Remove Vela customModels only (keep other settings)
