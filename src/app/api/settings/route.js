...
export async function PATCH(request) {
  try {
    ...
    // Migration 017 — the authUsers ROW is the authority; settings.password is
    // kept as a compatibility mirror. A password set here must reach the row, or
    // the login door (which verifies the row first) would reject the new
    // credential it was just told about. The first-ever set mints the seat.
    if (Object.prototype.hasOwnProperty.call(body, "password") && body.password) {
      try {
        const { DEFAULT_USERNAME } = await import("@/lib/auth/users.js");
        const { listUsers, setUserPassword, createUser } = await import("@/lib/db/repos/usersRepo.js");
        const users = await listUsers();
        if (users[0]) {
          await setUserPassword(users[0].id, body.password);
        } else {
          await createUser({ id: crypto.randomUUID(), username: DEFAULT_USERNAME, passwordHash: body.password });
        }
      } catch (err) {
        console.error("[settings] credential row sync failed:", err?.message || err);
      }
    }
