...
export default function ProfilePage() {
    ...
    fetch("/api/settings")
      ...
      .then((data) => {
        ...
        setSamlForm({
          ...
          samlIssuer: data?.samlIssuer || "urn:Vela:sp",
  ...
  const handleExportDatabase = async (password) => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `Vela-backup-${stamp}.json`;
    ...
    <div className="max-w-2xl mx-auto px-4 sm:px-0">
      <div className="flex flex-col gap-6">
          ...
          <div className="flex flex-col gap-3 pt-4 border-t border-border">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between p-3 rounded-lg bg-bg border border-border gap-2">
              <div>
                ...
                <p className="text-xs sm:text-sm text-text-muted font-mono break-all">~/.vela/db/data.sqlite</p>
              </div>
            </div>
                ...
                {!settings.hasPassword && (
                  <div className="p-3 rounded-lg bg-blue-500/10 border border-blue-500/20">
                    <p className="text-sm text-blue-600 dark:text-blue-400">
                      Setting your first dashboard password. Leave the current
                      password empty. Remote access stays locked until a
                      password is set.
                    </p>
                  </div>
                )}
                        ...
                        <div className="p-2.5 rounded border border-primary/20 bg-primary/5 text-primary text-xs">
                          ...
                          <ul className="list-disc pl-4 space-y-1 font-mono text-[11px]">
                            ...
                            <li>
                              ...
                              <code className="bg-bg px-1 py-0.5 rounded break-all">{samlForm.samlIssuer || "urn:Vela:sp"}</code>
                            </li>
                            <li>
                              <b>NameID Format:</b>{" "}
        ...
        <div className="text-center text-xs sm:text-sm text-text-muted py-4">
          ...
          <p className="mt-1">{isRemoteHost ? "Remote Mode - served through this host" : "Local Mode - All data stored on your machine"}</p>
        </div>
      </div>
