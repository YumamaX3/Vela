...
function getStatus(status) {
  ...
  if (status.hasVela) return { label: "Connected", cls: "bg-green-500/10 text-green-600 dark:text-green-400" };
...
export default function ToolSummaryCard({ toolId, tool, status }) {
  ...
  return (
    <Link href={`/dashboard/cli-tools/${toolId}`} className="block">
      <Card padding="sm" className="h-full overflow-hidden hover:border-primary/50 motion-control cursor-pointer">
