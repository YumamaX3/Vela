...
//
// Three layers, each closing a distinct bypass class (ported from upstream
// 9router b870b5d4, #3714 — W1 divergence wave, v0.9.47):
//   1. assertPublicUrl        — synchronous literal-IP/hostname checks (cheap,
...
//                               *resolves* to a private/loopback address (a
//                               nip.io/sslip.io wildcard-DNS domain, or an
//                               attacker's own A record pointed at 127.0.0.1)
//                               is also rejected.
//   3. fetchPublic            — wraps fetch() with manual redirect handling so
...
//
// Layer 1 alone previously had matching bugs, not just missing coverage:
// hostname checks ran on the raw string without normalizing a trailing dot
// ("localhost."), and the IPv6 check only recognized one textual representation
// of an IPv4-mapped address (dotted "::ffff:a.b.c.d") while Node/WHATWG URL
// parsing can normalize the same address to hex form ("::ffff:7f00:1") — a
...
function isBlockedIpv6Groups(g) {
  ...
  // IPv4-compatible ::a.b.c.d/96 (deprecated, still parseable) — excludes ::
  // and ::1, which already matched above.
...
// domain that merely *resolves* to a private/loopback/metadata address
// (wildcard-DNS services like nip.io/sslip.io, or an attacker-controlled
// domain with an A record pointed at 127.0.0.1) is rejected too, not just
// IPs typed directly into the URL.
export async function assertPublicUrlResolved(rawUrl) {
  const parsed = new URL(rawUrl);
  const host = normalizeHost(parsed.hostname);
  if (isBlockedHost(host)) throw new Error("Blocked URL: internal host");

  // Already a literal IPv4/IPv6 address — isBlockedHost above already covered
  // it, no DNS lookup applies (and dns.lookup would just echo it back anyway).
