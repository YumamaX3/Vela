...
Integrate Vela with Cline VSCode extension to route your AI requests through Vela's intelligent routing system.
...
Set the base URL to your Vela endpoint:
...
**Steps:**
1. In the **Base URL** field, enter your Vela endpoint
2. Make sure to include `/v1` at the end

### 4. Add API Key

1. In the **API Key** field, enter your Vela API key
2. You can find your API key in the Vela dashboard under **Settings → API Keys**
3. The key should start with `sk-Vela-`

### 5. Select Model

1. In the **Model** dropdown, you can either:
   - Select from available models (if Cline auto-detects them)
   - Manually enter the model name from your Vela configuration
...
You can use any model configured in your Vela dashboard. Common examples:
...
4. Cline will use Vela to process your request

### Code Generation

1. Ask Cline to generate code: "Create a React component for a login form"
2. Cline will generate code using Vela
...
3. Get AI-powered explanations through Vela

### File Operations

1. Ask Cline to create, modify, or delete files
2. Cline will use Vela to understand context and make changes
...
1. Verify your API key in Vela dashboard
2. Make sure you copied the entire key including the `sk-Vela-` prefix
3. Check that the API key has not expired
4. Try regenerating a new API key

### "Model Not Found" Error

1. Verify the model name matches exactly with your Vela configuration
...
2. Verify your Vela instance is running and healthy
3. Try reloading VSCode window (Cmd/Ctrl + Shift + P → "Reload Window")
4. Check Vela logs for any errors
...
1. In Cline settings, set Base URL to: `https://vela.my.id`
2. Make sure you have configured your API key in the Vela cloud dashboard
...
2. **Monitor Usage**: Check Vela dashboard for usage statistics and costs
...
## Integration with Vela Features

### Model Routing

Vela automatically routes your requests to the best available provider based on:
- Model availability
...
If a provider fails, Vela automatically falls back to alternative providers configured in your dashboard.

### Usage Tracking

Monitor your Cline usage through Vela dashboard:
- Total requests
- Token usage
- Cost per model
