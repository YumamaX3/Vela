...
Tích hợp Vela với extension Cline VSCode để định tuyến request AI qua hệ thống routing thông minh của Vela.
...
Đặt base URL tới endpoint Vela:
...
**Các bước:**
1. Trong field **Base URL**, nhập endpoint Vela
2. Đảm bảo bao gồm `/v1` ở cuối

### 4. Thêm API Key

1. Trong field **API Key**, nhập API key Vela của bạn
2. Bạn có thể tìm API key trong Vela dashboard tại **Settings → API Keys**
3. Key bắt đầu bằng `sk-Vela-`

### 5. Chọn Model

1. Trong dropdown **Model**, bạn có thể:
   - Chọn từ model có sẵn (nếu Cline auto-detect)
   - Nhập tên model thủ công từ cấu hình Vela
...
Bạn có thể dùng bất kỳ model nào đã cấu hình trong Vela dashboard. Ví dụ phổ biến:
...
3. Nhận giải thích AI qua Vela

### Thao tác File

1. Yêu cầu Cline tạo, sửa hoặc xóa files
2. Cline sẽ dùng Vela để hiểu context và thực hiện thay đổi
...
1. Xác minh API key trong Vela dashboard
2. Đảm bảo bạn sao chép đầy đủ key bao gồm prefix `sk-Vela-`
3. Kiểm tra API key chưa hết hạn
4. Thử tạo API key mới

### Lỗi "Model Not Found"

1. Xác minh tên model khớp chính xác với cấu hình Vela
...
2. Xác minh Vela instance đang chạy và healthy
3. Thử reload cửa sổ VSCode (Cmd/Ctrl + Shift + P → "Reload Window")
4. Kiểm tra logs Vela để xem lỗi
...
2. **Theo dõi Usage**: Kiểm tra Vela dashboard để xem thống kê và chi phí
...
## Tích hợp với Tính năng Vela

### Định tuyến Model

Vela tự động định tuyến request đến provider tốt nhất hiện có dựa trên:
...
Nếu một provider thất bại, Vela tự động fallback sang provider khác đã cấu hình trong dashboard.

### Theo dõi Usage

Giám sát usage Cline qua Vela dashboard:
- Tổng request
- Token usage
- Chi phí mỗi model
- Phân bổ provider
