...
Despliega Vela en VPS o Docker para acceso remoto y uso en producción.
...
| `DATA_DIR` | `~/.Vela` | Ruta de almacenamiento de la base de datos |
...
# Iniciar Vela con PM2
pm2 start npm --name Vela -- start
...
Crea `/etc/nginx/sites-available/Vela`:
...
# Si NO usas proxy reverso, permite los puertos de Vela
sudo ufw allow 3000/tcp
sudo ufw allow 32060/tcp
...
tar -czf Vela-backup-$(date +%Y%m%d).tar.gz /var/lib/Vela

# Respaldo automatizado diario (agregar a crontab)
0 2 * * * tar -czf /backups/Vela-$(date +\%Y\%m\%d).tar.gz /var/lib/Vela
```
...
# Conexiones de red
netstat -tulpn | grep -E '3000|32060'
```
...
```bash
# Verificar si Vela está corriendo
pm2 status
...
sudo chown -R $USER:$USER /var/lib/Vela
chmod 755 /var/lib/Vela
```
