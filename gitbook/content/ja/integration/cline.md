...
VelaをCline VSCode拡張機能と統合し、AIリクエストをVelaのインテリジェントルーティングシステム経由でルーティングします。

## 前提条件

- Visual Studio Codeがインストール済み
- VSCodeマーケットプレイスからCline拡張機能がインストール済み
- Velaがローカルで動作中、またはクラウドエンドポイントが設定済み
- VelaダッシュボードからのAPIキー
...
**手順:**
1. **Base URL** フィールドにVelaエンドポイントを入力
2. 末尾に `/v1` を必ず含める

### 4. APIキーを追加

1. **API Key** フィールドにVela APIキーを入力
2. APIキーはVelaダッシュボードの **Settings → API Keys** で確認できます
3. キーは `sk-Vela-` で始まります
...
Velaダッシュボードで設定されたモデルを使用できます。一般的な例:
...
2. ClineはVelaを使用してコンテキストを理解し変更を加える
3. 受け入れる前に変更を確認
...
1. Velaが動作中か確認: `curl http://localhost:32060/health`
2. Base URLが正しく、`/v1` を含むことを確認
3. ファイアウォールがポート32060をブロックしていないか確認
4. VSCodeを再起動してみる

### 「Invalid API Key」エラー

1. VelaダッシュボードでAPIキーを確認
2. `sk-Vela-` プレフィックスを含むキー全体をコピーしたか確認
3. APIキーが期限切れでないか確認
4. 新しいAPIキーを再生成してみる

### 「Model Not Found」エラー

1. モデル名がVela設定と正確に一致するか確認
2. Velaダッシュボードでプロバイダー接続がアクティブか確認
...
2. **使用量をモニター**: Velaダッシュボードで使用統計とコストを確認
...
Velaは以下に基づいて最適な利用可能なプロバイダーにリクエストを自動的にルーティング:
- モデル可用性
- プロバイダーヘルスステータス
- コスト最適化
- ロードバランシング
...
プロバイダーが失敗した場合、Velaは自動的にダッシュボードで設定された代替プロバイダーにフォールバックします。

### 使用量トラッキング

Velaダッシュボード経由でCline使用量をモニター:
- 総リクエスト数
- トークン使用量
- モデルごとのコスト
- プロバイダー分布
