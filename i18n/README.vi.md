...
🎉 Bảng điều khiển (Dashboard) sẽ tự động mở tại `http://localhost:32060`
...
```bash
npm run build
PORT=32060 HOSTNAME=0.0.0.0 NEXT_PUBLIC_BASE_URL=http://localhost:32060 npm run start
```

URL mặc định:
- Bảng điều khiển Dashboard: `http://localhost:32060/dashboard`
- API tương thích OpenAI: `http://localhost:32060/v1`
...
Settings → Models → Advanced:
  OpenAI API Base URL: http://localhost:32060/v1
...
```json
{
  "anthropic_api_base": "http://localhost:32060/v1",
...
```bash
export OPENAI_BASE_URL="http://localhost:32060"
...
export NEXT_PUBLIC_BASE_URL="http://localhost:32060"
...
| `PORT` | framework default | Cổng dịch vụ (`32060` trong các ví dụ) |
| `HOSTNAME` | framework default | Bind host (Docker mặc định là `0.0.0.0`) |
| `NODE_ENV` | runtime default | Đặt `production` để triển khai |
| `BASE_URL` | `http://localhost:32060` | URL cơ sở nội bộ phía máy chủ được sử dụng bởi các tác vụ đồng bộ đám mây |
...
- Đặt `PORT=32060` và `NEXT_PUBLIC_BASE_URL=http://localhost:32060`

**Lỗi đồng bộ đám mây**
- Xác minh `BASE_URL` trỏ đến phiên bản đang chạy của bạn (ví dụ: `http://localhost:32060`)
...
```bash
POST http://localhost:32060/v1/chat/completions
...
```bash
GET http://localhost:32060/v1/models
Authorization: Bearer your-api-key
...
- cục bộ (`http://127.0.0.1:32060/v1/chat/completions`): hoạt động với `stream=false` và `stream=true`.
