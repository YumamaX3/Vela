...
🎉 Панель управления откроется на `http://localhost:32060`
...
URL по умолчанию:
- Панель управления: `http://localhost:32060/dashboard`
- OpenAI-совместимый API: `http://localhost:32060/v1`
...
```json
{
  ...
  "models": {
    "providers": {
      "9router": {
        "baseUrl": "http://127.0.0.1:32060/v1",
        "apiKey": "sk_9router",
...
| `PORT` | framework default | Порт сервиса (`32060` в примерах) |
...
| `BASE_URL` | `http://localhost:32060` | Внутренний серверный базовый URL для задач облачной синхронизации |
...
- Установите `PORT=32060` и `NEXT_PUBLIC_BASE_URL=http://localhost:32060`

**Ошибки облачной синхронизации**
- Убедитесь, что `BASE_URL` указывает на ваш работающий инстанс (например, `http://localhost:32060`)
...
- Локально (`http://127.0.0.1:32060/v1/chat/completions`): работает с `stream=false` и `stream=true`.
