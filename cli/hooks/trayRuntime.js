...
function cleanupLegacySystray({ silent = false } = {}) {
  // 1) Runtime dir: ~/.vela/runtime/node_modules/systray (or %APPDATA% on Win)
  // 2) npm global nested: <npm_prefix>/node_modules/vela/node_modules/systray
  ...
  for (const dir of targets) {
    if (fs.existsSync(dir)) {
      try {
        ...
        if (!silent) console.log(`[vela][runtime] removed legacy systray: ${dir}`);
      } catch (e) {
        if (!silent) console.warn(`[vela][runtime] failed to remove ${dir}: ${e.message}`);
      }
    }
  }
}
...
function chmodSystrayBin({ silent = false } = {}) {
  ...
  try {
    ...
    if (!silent) console.warn(`[vela][runtime] chmod tray bin failed: ${e.message}`);
  }
}
