...
describe("Wave B2 — migration 005 + posture refusals", () => {
  it("a fresh DB migrates to schemaVersion 5 with backupLedger", async () => {
    await freshDb();
    const { getAdapter } = await import("@/lib/db/driver.js");
    const adapter = await getAdapter();
    expect(adapter.get(`SELECT value FROM _meta WHERE key = 'schemaVersion'`).value).toBe("5");
