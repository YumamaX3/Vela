...
describe("tool-result images reach Anthropic-compatible and Command Code upstreams", () => {
  ...
  it("sends an image block to Command Code instead of a placeholder", () => {
    ...
    // Upstream pins `"mediaType":"image/png"` here, but that field is a duplicate
    // of mimeType that their own translator NEVER produces: the comment at the top
    // of open-sse/translator/request/openai-to-commandcode.js and three sibling
    // tests (openai-to-commandcode.test.js ×2, bugs-gemini-cursor-commandcode)
    // all pin {type, image, mimeType}. The block's mime is what this test must
    // hold; the redundant key was an upstream self-contradiction (their code
    // emits it, no test of theirs agrees). v0.5.81 port, 2026-09-19.
