...
describe("OpenCode Free User-Agent Validation", () => {
  // Vela's canonical caller shape: the version prefix the gate validates, plus
  // the ai-sdk/runtime tail the real CLI emits (upstream v0.5.81 sends the bare
  // version). The tail is deliberate — it is the faithful client fingerprint.
  const CANONICAL_UA = "opencode/1.18.31 ai-sdk/provider-utils/4.0.46 runtime/bun/1.3.14";

  it("defaults User-Agent to the canonical opencode fingerprint for non-opencode downstream clients", () => {
    const executor = getExecutor("opencode");
    const headersNoUa = executor.buildHeaders({});
    expect(headersNoUa["User-Agent"]).toBe(CANONICAL_UA);

    const headersClaude = executor.buildHeaders({ rawHeaders: { "user-agent": "Claude-Code/1.0" } });
    expect(headersClaude["User-Agent"]).toBe(CANONICAL_UA);
  });
  ...
  it("replaces bare opencode with versioned opencode/1.18.31 to prevent 403 FreeTierError", () => {
    ...
    expect(headers["User-Agent"]).toBe(CANONICAL_UA);
  });
